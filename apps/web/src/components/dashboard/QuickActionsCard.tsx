...
export function QuickActionsCard() {
  return (
    <div className="rounded-card border border-line bg-surface p-4 shadow-control">
      <h3 className="mb-2 text-sm font-bold text-ink">Acciones Rápidas</h3>
      <ul className="space-y-2">
        {ACTIONS.map(({ label, icon: Icon }) => (
          <li key={label}>
            <button
              type="button"
              className="flex w-full items-center gap-2.5 rounded-control bg-brand-600/8 px-3.5 py-2.5 text-sm font-semibold text-brand-700 transition hover:bg-brand-600/14"
            >
              <Icon size={16} />
